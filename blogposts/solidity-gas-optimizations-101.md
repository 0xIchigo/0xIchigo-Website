...
### Unnecessary Events
What's worse than over-indexing an event? Emitting unnecessary events. Logging opcodes can be quite expensive, so don't emit events for easily accessible data such as the value of a state variable or `block.timestamp`. Moreover, avoid redundant events such as emitting an event for when tokens are minted while the underlying function/library already emits such an event.
...
You can also cut out 10 opcodes in the creation-time EVM bytecode if you declare a `constructor()` as `payable`. This is due to the fact `payable` removes the need for an inital check of `msg.value == 0`, saving 13 gas on deployment with no security risks.
